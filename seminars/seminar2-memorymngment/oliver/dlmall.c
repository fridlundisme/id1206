...
/*struct head *merge(struct head *block) {
    ...
    if (block->bfree) {
        ...
        int size = (block->size + before(block)->size);
        before(block)->size = size;

        aft->prev = before(block);
        aft->bsize = size;

        block = before(block);
        printf("Blockmerge size: %d\n", block->size);
    }

    if (aft->free) {
        detatch(aft);

        int size = (block->size + aft->size);
        block->size = size;
        printf("Blockmerge size: %d\n", block->size);
        after(aft)->prev = block;
