...
#define BSIZE_ADDRESS(block) (int*)(block - 8)
...
struct head *before(struct head *block){
    return (struct head*)((char*)block - HEAD - bsize(block));
}

uint16_t bsize(struct head *block){
    return *((char*)block - sizeof(uint64_t));
}

void writeBsize(struct head uint16_t bsize){
